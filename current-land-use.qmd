...
rb_landuse_map <- mapview(
  rock_landuse_zoning,
  layer.name = "Land use",
  zcol = "category",
  col.regions = RColorBrewer::brewer.pal(8, "Paired")
  )
...
```{r}
#| label: fig-lu-map
#| fig-cap: "Land use categories in Rockbridge County"
...
```{r}
#| label: fig-lu-res-map
#| fig-cap: "Residential land use groups in Rockbridge County"

rock_landuse_zoning |> 
  filter(category == "Residential") |> 
  mapview(
    layer.name = "Residential use",
    zcol = "group",
    col.regions = RColorBrewer::brewer.pal(4, "Paired")
    )
